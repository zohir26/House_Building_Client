...
const Apartments = () => {
  ...
  useEffect(() => {
    const fetchApartments = async () => {
      try {
        ...
        const res = await axiosSecure.get(
          `/apartmentPage?page=${currentPage}&limit=${limit}` +
            (filterApplied ? `&min=${priceFilter.min}&max=${priceFilter.max}` : '')
        );
        setApartments(res.data.apartments || []);
        setTotalPages(res.data.totalPages || 1);
        setLoading(false);
    ...
    fetchApartments();
  }, [currentPage, axiosSecure, filterApplied]);
  ...
  if (error) return <div className="text-center text-lg font-bold text-red-500">{error}</div>;
